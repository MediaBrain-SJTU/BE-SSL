...
class CandiBaseTrainer(nnUNetTrainerV2):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None, unpack_data=True, deterministic=True, fp16=False):
        ...
        self.num_val_batches_per_epoch = 17
        self.save_every = 10
        self.ratio = 1.0
    ...
    def initialize_optimizer_and_scheduler(self):
        ...
        self.optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, self.network.parameters()), self.initial_lr, weight_decay=self.weight_decay,
                                          amsgrad=True)
    ...
    def do_split(self):
        ...
        if self.fold == "all":
            ...
            self.tr_keys = self.val_keys = list(self.dataset.keys())
        else:
            ...
            if self.fold < len(splits):
                self.tr_keys = splits[self.fold]['train']
                self.val_keys = splits[self.fold]['val']
                self.print_to_log_file("This split has %d training and %d validation cases."
                                       % (len(self.tr_keys), len(self.val_keys)))
            else:
        ...
        for i in self.tr_keys[:int(len(self.tr_keys)*self.ratio)]:
            self.dataset_tr[i] = self.dataset[i]
        self.dataset_val = OrderedDict()
        for i in self.val_keys:
            self.dataset_val[i] = self.dataset[i]

        print('train keys: ', self.dataset_tr.keys())
        print('val keys: ', self.dataset_val.keys())
    ...
    def preprocess_patient(self, input_files):
        from nnunet.preprocessing.cropping import load_case_from_list_of_files
        data, seg, properties = load_case_from_list_of_files(input_files)
